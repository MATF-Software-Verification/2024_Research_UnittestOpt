...
class Optimisation:
    def __init__(self, optimisation_type: Union['random', 'genetic', 'bayesian'],
                 optimisation_config: OptimisationConfig, algorithm_config: Union[
                GeneticConfig]):
    ...
    def run(self, project_path):
        ...
        if self.optimisation_type == 'random':
            optimisation = RandomOptimisation(coverage_data_list=coverage_data_list, coverage_data_handler=coverage_data_handler, algorithm_config=self.algorithm_config, optimisation_config=self.optimisation_config)
        elif self.optimisation_type == 'genetic':
            optimisation = GeneticOptimisation(coverage_data_list=coverage_data_list, coverage_data_handler=coverage_data_handler, algorithm_config=self.algorithm_config, optimisation_config=self.optimisation_config)
